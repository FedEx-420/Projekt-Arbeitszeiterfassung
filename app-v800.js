...
(() => {
  ...
  function showInvoicePreviewControls(windowRef) {
    ...
    const addControls = () => {
      ...
      bar.style.cssText = 'position:sticky;top:0;z-index:10;display:flex;gap:10px;align-items:center;padding:12px 0;background:#fff;border-bottom:1px solid #d9e5e2;margin-bottom:18px;font:14px Arial,sans-serif';
  ...
  function printInvoicePdf(group) {
    ...
    const fields = customer?.custom_fields && typeof customer.custom_fields === 'object' ? customer.custom_fields : {};
    const customerName = customer?.name || group.customerName || 'Kunde';
    const customerAddress = [
      [fields.street, fields.house_no].filter(Boolean).join(' '),
      [fields.postal_code, fields.city].filter(Boolean).join(' '),
      fields.email
    ].filter(Boolean);
    const first = [...group.orders].sort((left, right) => String(left.work_date).localeCompare(String(right.work_date)))[0];
    const last = [...group.orders].sort((left, right) => String(right.work_date).localeCompare(String(left.work_date)))[0];
    const orderNumber = String(first?.id || '').replace(/[^a-z0-9]/gi, '').slice(0, 8).toUpperCase() || 'OFFEN';
    const invoiceNumber = `RE-${today().replaceAll('-', '')}-${orderNumber}`;
    const employeeInfo = order => {
      const person = state.rows.people.find(row => same(row.id, order.employee_id));
      return {
        name: person?.display_name || person?.username || 'Mitarbeiter nicht verfügbar',
        time: `${timeText(order.start_time)} – ${timeText(order.end_time)}`,
        hours: h(order.executed_hours)
      };
    };
    const workSummaryRows = group.orders.map(order => {
      const employee = employeeInfo(order);
      return `<div style="padding:10px 0;border-bottom:1px solid #d9e5e2"><b>Mitarbeiter: ${escape(employee.name)}</b><br><span>Arbeitszeit: ${escape(employee.time)} · Ausgeführte Stunden: ${escape(employee.hours)}</span></div>`;
    }).join('');
    const itemRows = group.orders.flatMap(order => {
      const employee = employeeInfo(order);
    ...
    windowRef.document.write(`<!doctype html><html lang="de"><head><meta charset="utf-8"><title>Rechnung ${escape(invoiceNumber)}</title><style>body{font:14px Arial,sans-serif;color:#173531;padding:34px;max-width:900px;margin:auto}header{display:flex;justify-content:space-between;gap:28px;border-bottom:3px solid #238473;padding-bottom:22px;margin-bottom:30px}.logo{max-width:220px;max-height:95px;object-fit:contain;display:block;margin-bottom:8px}.company{font-size:15px;line-height:1.5}.title{font-size:31px;color:#163e37;margin:0 0 7px}.muted{color:#59716c}.address{white-space:pre-line;line-height:1.55;margin:24px 0 30px}.meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px 28px;background:#edf6f3;padding:16px;margin:0 0 26px}.meta b{display:block;font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:#48645d}table{width:100%;border-collapse:collapse;margin-top:14px}th,td{padding:10px 8px;border-bottom:1px solid #d9e5e2;text-align:left;vertical-align:top}th{background:#eaf4f1;font-size:12px}td:nth-child(3),td:nth-child(4),td:nth-child(5),th:nth-child(3),th:nth-child(4),th:nth-child(5){text-align:right}small{display:block;color:#5c716c;margin-top:3px}.total{margin:22px 0 0 auto;width:320px;border-top:2px solid #238473;padding-top:10px;display:flex;justify-content:space-between;font-size:18px}.footer{margin-top:44px;border-top:1px solid #d9e5e2;padding-top:14px;color:#5c716c;font-size:12px}@media print{body{padding:0}header{margin-top:0}#invoice-preview-actions,#pdf-return-actions{display:none!important}}</style></head><body><header><div>${logo ? `<img class="logo" src="${escape(logo)}" alt="Firmenlogo">` : ''}<div class="company"><b>${escape(company.company_name || 'Zeiterfassung')}</b><br>Rechnungsausgabe</div></div><div><h1 class="title">Rechnung</h1><div class="muted">${escape(invoiceNumber)}</div></div></header><div class="address"><b>Rechnung an</b><br>${escape([fields.first_name, customerName].filter(Boolean).join(' ') || customerName)}${customerAddress.length ? `<br>${customerAddress.map(escape).join('<br>')}` : ''}</div><section class="meta"><div><b>Rechnungsdatum</b>${dateText(today())}</div><div><b>Leistungszeitraum</b>${dateText(first?.work_date)}${same(first?.work_date, last?.work_date) ? '' : ` bis ${dateText(last?.work_date)}`}</div><div><b>Enthaltene Arbeitsscheine</b>${group.orders.length}</div><div><b>Kunde</b>${escape(customerName)}</div></section><table><thead><tr><th>Datum</th><th>Position</th><th>Menge</th><th>Einzelpreis</th><th>Gesamt</th></tr></thead><tbody>${itemRows}</tbody></table><div class="total"><b>Rechnungsbetrag</b><b>${money(total)}</b></div><p class="footer">Diese Rechnung wurde automatisch aus ${group.orders.length} Arbeitsschein(en) erstellt.</p></body></html>`);
    windowRef.document.close();
    addPdfReturnBar(windowRef);
    const workSummary = windowRef.document.createElement('section');
    workSummary.style.cssText = 'margin:0 0 24px;padding:16px;background:#f6faf9;border-left:4px solid #238473';
    workSummary.innerHTML = `<b style="display:block;margin-bottom:8px">Ausführung durch Mitarbeiter</b>${workSummaryRows}`;
    const invoiceTable = windowRef.document.querySelector('table');
    if (invoiceTable) invoiceTable.before(workSummary);
    const positionHeader = windowRef.document.querySelector('thead th:nth-child(2)');
    if (positionHeader) positionHeader.textContent = 'Leistung / Mitarbeiter / Arbeitszeit';
    return windowRef;
  }
  function printBillingPdf(key, invoiced) {
    ...
    windowRef.document.write(`<!doctype html><title>Zusammengefasster Arbeitsschein</title><style>body{font:14px Arial;padding:24px;color:#183431}h1{margin:0 0 4px}h2{margin:26px 0 8px;padding-top:18px;border-top:1px solid #d7e3e0}h3{font-size:14px;margin:14px 0 5px}p,li{line-height:1.55}ul{margin:0;padding-left:20px}.summary{background:#edf5f3;padding:14px;border-radius:8px}@media print{#pdf-return-actions{display:none!important}}</style><h1>Zusammengefasster Arbeitsschein</h1><p>${escape(managerBusiness()?.company_name || 'Zeiterfassung')}<br><b>Kunde:</b> ${escape(group.customerName)}<br><b>Status:</b> ${invoiced ? 'Bereits abgerechnet' : 'Offen zur Abrechnung'}</p><p class="summary"><b>${group.orders.length} Arbeitsscheine</b><br>Arbeitsstunden gesamt: ${h(totalHours)}<br>Material gesamt: ${money(totalMaterial)}</p>${detailRows}<p class="summary"><b>Gesamtsumme aller enthaltenen Arbeitsscheine: ${money(totalMaterial)}</b></p><script>window.onload=()=>window.print()<\/script>`); windowRef.document.close(); addPdfReturnBar(windowRef);
  }
  function printOrderPdf(orderId) {
    ...
    const money = value => n(value).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' });
    const materialRows = items.map(item => { const price = invoiceItemPrice(item, order); return `<tr><td>${escape(invoiceItemName(item, order))}</td><td>${n(item.quantity).toLocaleString('de-DE')}</td><td>${money(price)}</td><td>${money(n(item.quantity) * price)}</td></tr>`; }).join('');
    const total = items.reduce((sum, item) => sum + n(item.quantity) * invoiceItemPrice(item, order), 0), documentation = String(order.documentation || '').trim();
    const windowRef = window.open('', '_blank'); if (!windowRef) throw new Error('Bitte Pop-ups erlauben, um die PDF zu erstellen.');
    windowRef.document.write(`<!doctype html><title>Arbeitsschein</title><style>body{font:14px Arial;padding:24px;color:#183431}h1{margin:0 0 4px}h2{margin-top:28px;font-size:17px}p{line-height:1.55}table{width:100%;border-collapse:collapse}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left}th{background:#edf5f3}.total{font-weight:bold;text-align:right}@media print{#pdf-return-actions{display:none!important}}</style><h1>Arbeitsschein</h1><p>${escape(managerBusiness()?.company_name || 'Zeiterfassung')}<br>Mitarbeiter: ${escape(person?.username || '')}</p><h2>${escape(order.customer_name || 'Ohne Kunde')}</h2><p><b>Datum:</b> ${dateText(order.work_date)}<br><b>Beschreibung:</b> ${escape(order.title || '—')}<br><b>Arbeitszeit:</b> ${timeText(order.start_time)} bis ${timeText(order.end_time)}<br><b>Pause:</b> ${h(order.pause_hours)}<br><b>Ausgeführte Stunden:</b> ${h(order.executed_hours)}</p>${documentation ? `<h2>Dokumentation</h2><p>${escape(documentation).replace(/\n/g, '<br>')}</p>` : ''}${items.length ? `<h2>Material</h2><table><thead><tr><th>Artikel</th><th>Menge</th><th>Preis</th><th>Gesamt</th></tr></thead><tbody>${materialRows}<tr><td colspan="3" class="total">Material gesamt</td><td><b>${money(total)}</b></td></tr></tbody></table>` : ''}<p class="total">Gesamtsumme dieses Arbeitsscheins: ${money(total)}</p><script>window.onload=()=>window.print()<\/script>`); windowRef.document.close(); addPdfReturnBar(windowRef);
  }
  function printPdf() { const person = worker(), id = workerId(); const lines = state.rows.entries.filter(row => same(row.employee_id, id)).map(row => `<tr><td>${dateText(row.work_date)}</td><td>${escape(row.customer_name)}</td><td>${timeText(row.start_time)}</td><td>${timeText(row.end_time)}</td><td>${h(row.pause_hours)}</td><td>${h(row.executed_hours)}</td></tr>`).join(''); const windowRef = window.open('', '_blank'); if (!windowRef) throw new Error('Bitte Pop-ups erlauben, um die PDF zu erstellen.'); windowRef.document.write(`<!doctype html><title>Zeiterfassung</title><style>body{font:14px Arial;padding:24px}table{width:100%;border-collapse:collapse}th,td{padding:8px;border-bottom:1px solid #ddd;text-align:left}@media print{#pdf-return-actions{display:none!important}}</style><h1>Zeiterfassung – ${escape(person?.username)}</h1><p>Arbeitsstunden: ${h(state.rows.entries.filter(row => same(row.employee_id, id)).reduce((sum, row) => sum + n(row.executed_hours), 0))}<br>Überstunden: ${h(overtime(id))}<br>Urlaub übrig: ${vacationLeft(id)} Tage<br>Krankheitstage: ${annualSick(id)} Tage</p><table><thead><tr><th>Datum</th><th>Kunde</th><th>Von</th><th>Bis</th><th>Pause</th><th>Stunden</th></tr></thead><tbody>${lines}</tbody></table><script>window.onload=()=>window.print()<\/script>`); windowRef.document.close(); addPdfReturnBar(windowRef); }
