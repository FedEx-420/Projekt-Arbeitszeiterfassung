...
(() => {
  ...
  const state = {
    ...
    businessId: '', employeeId: '', customerId: '', orderId: '', menu: false, vacationForm: false, appointmentForm: false, notice: null, busy: false,
  ...
  function ordersView() {
    ...
    const newOrder = locked(id) ? `<div class="locked">${escape(lockedText(id))}</div>` : `<section class="panel"><h3>Neuer Arbeitsschein</h3><form data-form="order" class="entry-form"><label class="wide">Kunde<input name="customer" required list="customers"></label><label class="wide">Beschreibung<input name="title" placeholder="Ausgeführte Arbeiten"></label><div class="wide" id="material-lines">${materialRow()}</div><button type="button" class="secondary wide" data-action="more-material">Weiteres Material</button><label>Arbeitsbeginn${timeInput('start', previous)}</label><label>Arbeitsende${timeInput('end', '')}</label><label>Pause in Stunden<input name="pause" type="number" min="0" step="0.25" value="0"></label><label>Ausgeführte Stunden<input name="hours" type="number" min="0.25" step="0.25" required></label><label class="wide">Dokumentation<textarea name="documentation" rows="4"></textarea></label><label class="wide">Dokumente hochladen<input name="documents" type="file" multiple accept="image/*,.pdf,.doc,.docx"></label><button class="primary wide">Arbeitsschein speichern</button></form>${customerList()}${materialList()}</section>`;
  ...
  async function ensureMaterial(value) {
    ...
    if (current) return current;
    const selected = chooseSimilar(name, state.rows.materials.filter(row => row.active !== false), 'Artikel'); if (selected) return selected;
  ...
  async function saveMaterials(form, order, replace = false) {
    ...
    const resolved = []; for (let index = 0; index < materials.length; index++) { const material = await ensureMaterial(materials[index].value); if (material) resolved.push({ material, quantity: Math.max(0.25, n(quantities[index]?.value || 1)) }); }
  ...
  root.addEventListener('click', event => {
    ...
    if (action === 'nav') { state.view = button.dataset.view; state.menu = false; state.vacationForm = false; state.orderId = ''; render(); return; }
