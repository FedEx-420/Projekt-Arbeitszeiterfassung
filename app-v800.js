...
(() => {
  ...
  function orderEditor(order) {
    ...
    const invoiceButton = isManager() && !order.invoiced ? `<button type="button" class="primary small" data-action="invoice-order" data-id="${order.id}">Rechnung schreiben</button>` : order.invoiced ? '<span class="badge">Bereits abgerechnet</span>' : '';
  ...
  root.addEventListener('click', event => {
    ...
    if (action === 'invoice-order') {
      ...
      printInvoicePdf(group);
      return perform('Die Rechnung wurde erstellt und die enthaltenen Arbeitsscheine als abgerechnet markiert.', async () => { for (const order of group.orders) await write('work_orders', { invoiced: true }, 'PATCH', `id=eq.${encodeURIComponent(order.id)}`); state.orderId = ''; state.view = 'invoices-paid'; });
    }
    if (action === 'invoice-group') { const group = invoiceGroups(false).find(item => same(item.key, state.billingKey)); if (!group) return; if (!confirm(`Rechnung über ${group.orders.length} offenen Arbeitsschein(e) für ${group.customerName} erstellen?`)) return; printInvoicePdf(group); return perform('Die Rechnung wurde erstellt und die enthaltenen Arbeitsscheine als abgerechnet markiert.', async () => { for (const order of group.orders) await write('work_orders', { invoiced: true }, 'PATCH', `id=eq.${encodeURIComponent(order.id)}`); state.billingKey = ''; state.view = 'invoices-paid'; }); }
  ...
  function printInvoicePdf(group) {
    ...
    const itemRows = group.orders.flatMap(order => {
      ...
      return items.map(item => `<tr><td>${dateText(order.work_date)}</td><td>${escape(item.position_name || 'Leistung')}${order.title ? `<small>${escape(order.title)}</small>` : ''}</td><td>${n(item.quantity).toLocaleString('de-DE')}</td><td>${money(item.unit_price)}</td><td>${money(n(item.quantity) * n(item.unit_price))}</td></tr>`);
    }).join('');
