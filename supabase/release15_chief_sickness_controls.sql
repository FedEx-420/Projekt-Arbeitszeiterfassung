-- Only the chief may create, alter, or remove a recorded sickness day.
-- A sickness day may coexist with an approved vacation day.
...
begin
  if auth.uid() is not null
     and not (select app_private.is_chief())
     and coalesce(old.sick, 0) <> 0 then
    raise exception 'Krankheitstage können nur vom Chef eingetragen oder entfernt werden.';
  end if;
