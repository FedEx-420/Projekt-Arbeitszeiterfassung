...
function appView() {
  ...
  return `<div class="app-shell"><header class="app-header"><div class="brand"><span>AZ</span><div><strong>Arbeitszeit</strong><small>${esc(state.profile.username)}</small></div></div><div class="header-actions"><button class="menu-toggle" data-action="menu-toggle" aria-expanded="${state.menuOpen}">Menü <span aria-hidden="true">▾</span></button><button class="logout" data-action="logout">Abmelden</button></div></header><button class="menu-backdrop ${state.menuOpen ? 'open' : ''}" data-action="menu-close" aria-label="Menü schließen"></button><aside class="menu-popover ${state.menuOpen ? 'open' : ''}" aria-label="Hauptmenü"><nav>${navigation()}</nav></aside>${view()}${state.toast ? `<div class="toast ${state.toast.error ? 'error' : ''}">${esc(state.toast.message)}</div>` : ''}${state.busy ? '<div class="busy">Wird gespeichert …</div>' : ''}</div>`
}
