...
(() => {
  ...
  function billingDetailView() {
    ...
    return `<section class="page-head"><div><span class="eyebrow">${invoiced ? 'Bereits abgerechnet' : 'Zusammengefasster Arbeitsschein'}</span><h2>${escape(group.customerName)}</h2><p>${group.orders.length} Arbeitsscheine · ${h(total)}</p></div><div class="actions"><button type="button" class="secondary" data-action="billing-pdf">PDF drucken / speichern</button><button type="button" class="secondary" data-action="close-billing">Zurück</button></div></section><section class="panel"><h3>Enthaltene Arbeitsscheine</h3>${group.orders.map(row => `<article class="row-card"><div><b>${dateText(row.work_date)} · ${escape(row.title || 'Arbeitsschein')}</b><span>${timeText(row.start_time)} – ${timeText(row.end_time)} · ${h(row.executed_hours)}</span></div><button type="button" class="secondary small" data-action="open-order" data-id="${row.id}">Einzeln öffnen</button></article>`).join('')}</section>${invoiced ? '' : `<section class="panel"><button type="button" class="primary" data-action="invoice-group">Alle ${group.orders.length} Arbeitsscheine als abgerechnet markieren</button></section>`}`;
  }
  ...
  root.addEventListener('click', event => {
    ...
    if (action === 'nav') { state.view = button.dataset.view; state.menu = false; state.vacationForm = false; state.orderId = ''; state.orderOrigin = 'orders'; state.billingKey = ''; render(); return; }
