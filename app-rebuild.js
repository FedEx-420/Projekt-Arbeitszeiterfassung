...
function calendarView() {
  ...
  return page(monthLabel, 'PERSÖNLICHER KALENDER', `${employeePicker()}<section class="calendar-controls"><button type="button" data-action="month-prev">←</button><button type="button" data-action="month-today">Heute</button><button type="button" data-action="month-next">→</button></section><section class="card form-grid"><form data-form="vacation-request" class="form-grid wide"><label>Urlaub von<input name="startDate" type="date" required></label><label>Urlaub bis<input name="endDate" type="date" required></label><div class="form-actions"><button class="secondary">Urlaub beantragen</button></div></form><form data-form="appointment-create" class="form-grid wide"><label>Termin am<input name="date" type="date" value="${state.date}" required></label><label>Kunde<input name="customer" list="customers"></label><label class="wide">Termin<input name="title" required placeholder="Kundentermin"></label><label class="wide">Notiz<textarea name="notes" rows="2"></textarea></label><div class="form-actions"><button class="secondary">Kundentermin vormerken</button></div></form></section><section class="calendar"><div class="calendar-head">${['Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa', 'So'].map(day => `<span>${day}</span>`).join('')}</div><div class="calendar-grid">${calendarDays(state.year, state.month).map(date => {
    ...
    return `<button type="button" class="${classes}" data-action="calendar-date" data-date="${date}"><strong>${dateObject(date).getDate()}</strong>${note ? `<em>${escapeHtml(note)}</em>` : ''}</button>`
  }).join('')}</div></section><section class="card slim"><h2>${dateText(state.date)}</h2>${dayActivities(employee)}</section>${customerDatalist()}`)
}
