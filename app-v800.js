...
(() => {
  ...
  function printInvoicePdf(group) {
    ...
    const itemRows = group.orders.flatMap(order => {
      ...
      const employeeName = person?.display_name || person?.username || 'Nicht zugeordnet';
      const workTime = `${timeText(order.start_time)} – ${timeText(order.end_time)} · ${h(order.executed_hours)}`;
