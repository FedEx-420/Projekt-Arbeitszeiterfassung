...
(() => {
  ...
  function isMonteurstunde(material) { return lower(typeof material === 'string' ? material : material?.name) === 'monteurstunde'; }
  ...
  async function ensureMonteurstunde(targetBusinessId = businessId()) {
    const current = state.rows.materials.find(row => same(row.business_id, targetBusinessId) && isMonteurstunde(row));
    if (current) return current;
    const created = await write('materials', { business_id: targetBusinessId, name: 'Monteurstunde', unit_price: 0, active: true });
  ...
  async function saveMaterials(form, order, replace = false) {
    ...
    const resolved = []; for (let index = 0; index < materials.length; index++) { const material = await ensureMaterial(materials[index].value, targetBusinessId); if (material && !isMonteurstunde(material)) resolved.push({ material, quantity: Math.max(0.25, n(quantities[index]?.value || 1)) }); }
  ...
  async function saveMonteurstunde(order, hours) {
    const material = await ensureMonteurstunde(materialBusinessId(order.employee_id));
    if (!material?.id) throw new Error('Die Materialposition „Monteurstunde“ konnte nicht angelegt werden.');
    await write('work_order_items', { work_order_id: order.id, material_id: material.id, position_name: 'Monteurstunde', quantity: Math.max(0.25, n(hours)), unit_price: n(material.unit_price) });
  }
  async function updateMonteurPrice(form) {
    const material = state.rows.materials.find(row => same(row.business_id, businessId()) && isMonteurstunde(row));
    if (!material) throw new Error('Die Position „Monteurstunde“ wurde nicht gefunden.');
  ...
  root.addEventListener('click', event => {
    ...
    if (action === 'delete-material') { const material = state.rows.materials.find(row => same(row.id, button.dataset.id)); if (isMonteurstunde(material)) { notice('Die Position „Monteurstunde“ ist geschützt und kann nicht gelöscht werden.'); render(); return; } return confirm('Material wirklich löschen?') && perform('Material wurde gelöscht.', () => remove('materials', `id=eq.${encodeURIComponent(button.dataset.id)}`)); }
